...
rpy_image = cv2.imread('../additions/rpy.png', cv2.COLOR_BGR2GRAY)
...
markers_in_db = real_values.keys()
...
while True:
    ...
    if len(corners) > 0:
        ...
        for (markerCorner, markerID) in zip(corners, ids):
            if markerID not in markers_in_db:
                continue
        ...
        cv2.putText(image, f"{int(fps)} fps", (image.shape[1] - 100, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                    (255, 255, 255), 2)
        image[rpy_image_y1:rpy_image_y2, rpy_image_x1:rpy_image_x2] = rpy_image[:, :, 0]
        cv2.imshow("Image preview", detected_markers)
        if record:
            out.write(detected_markers)
            f.write(f"{time.time()},{pos[0]},{pos[1]},{pos[2]},{roll},{pitch},{yaw}\n")
        if snapshot:
            snapshot_msg = f"{time.time()},{pos[0]},{pos[1]},{pos[2]},{roll},{pitch},{yaw}\n"
